...
import { toggleChat, clearChat } from '../../store/slices/chatSlice';
import { toggleQuizModal, clearQuiz } from '../../store/slices/quizSlice';
...
const NodeDetailsPanel = () => {
  ...
  useEffect(() => {
    ...
    dispatch(clearChat());
  }, [selectedNode?.id, examples, dispatch]);
        ...
        <div className="px-4 sm:px-6 py-2 border-b border-gray-100 bg-gray-50">
          <nav className="flex items-center overflow-x-auto scrollbar-hide text-xs">
            {pathNodes.map((path, index) => (
              <React.Fragment key={`path-${path.id}-${index}`}>
                {index > 0 && (
      ...
      <div className="border-t border-gray-200 p-4">
        <div className="flex space-x-2">
          <button
            ...
            className="flex-1 inline-flex justify-center items-center px-4 py-2 border border-transparent text-sm font-medium rounded-md shadow-sm text-white bg-sgmm-500 hover:bg-sgmm-600 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-sgmm-500"
          >
